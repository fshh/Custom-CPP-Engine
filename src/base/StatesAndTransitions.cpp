...
PatrolState::PatrolState(float speed, float x0, float y0, float x1, float y1, std::shared_ptr<AnimatorComponent> animator):
  mSpeed(speed),
  mX0(x0),
  mY0(y0),
  mX1(x1),
...
PatrolState::update(GameObject & gameObject, Level & level)
{
  ...
  // Move and change direction if end is reached
  if(moveToward(gameObject, tx, ty, mSpeed)) {
	  mForward = !mForward;
  }

  mAnimator->setState("run");
}



ChaseState::ChaseState(float speed, std::weak_ptr<GameObject> which, std::shared_ptr<AnimatorComponent> animator):
  mSpeed(speed),
  mWhich(which),
	mAnimator(animator)
{
...
MoveState::MoveState(float speed, float x, float y, std::shared_ptr<AnimatorComponent> animator):
  mSpeed(speed),
  mX(x),
  mY(y),
	mAnimator(animator)
