...
{
	auto collided = collisions.find(obj);
	if (collided == collisions.end()) {
		collisions.emplace(obj);
		for (auto genericComponent: mGenericComponents) {
...
GameObject::isColliding(const GameObject & obj) const
{
  ...
  bool result;
  {
  	PROFILE("SDL_IntersectRect");
    result = SDL_IntersectRect(&thisRect, &objRect, &outRect);
  }
  return result;
}
...
GameObject::isColliding(float px, float py) const
{
  ...
  bool result;
  {
  	PROFILE("SDL_PointInRect");
  	result = SDL_PointInRect(&point, &thisRect);
  }
  return result;
}
